...
let token, adminUser, otherAdminUser, user, userId, adminId, userToken,
    csrfToken, cookies, properties;
...
describe('/users', () => {
  ...
  describe('GET /', () => {
    ...
    const exec = async () => {
      return await request(app).get('/users').set('Cookie', `token=${token}`);
    };
  ...
  describe('GET / :id', () => {
    ...
    const getUser = async () => {
      return await request(app).get(`/users/${userId}`).set('Cookie', `token=${token}`);
    };

    const getAdmin = async () => {
      return await request(app).get(`/users/${adminUser._id}`).set('Cookie', `token=${token}`);
    };
  ...
  describe('DELETE / ', ()=> {
    ...
    it('should return 403 with invalid csrf token', async ()=> {
      ...
      const res = await request(app).delete('/users')
                          ...
                          .type('form')
                          .send({ id: userId.toHexString(),
                                  _csrf: 'hreuhgiahrgiuhaergiheariugh'});
      expect(res.status).toBe(403);
      let count = await User.find().countDocuments();
      expect(count).toEqual(3);
    });
  ...
  describe('POST / upgrade', () => {
    const getUser = async () => {
      ...
      let $ = cheerio.load(res.text);
      csrfToken = $('.upgrade').find('[name=_csrf]').val();
      cookies = res.headers['set-cookie'];
      cookies.push(`token=${token}`);
    };
    ...
    it('should update isAdmin as true for a user when logged in as an admin', async () => {
      await getUser();
      const res = await request(app).post('/users/upgrade')
                          .set('Cookie', cookies)
                          .type('form')
                          .send({ id: userId.toHexString(),
                                  _csrf: csrfToken});
  ...
  describe('POST / downgrade', () => {
    const getAdmin = async () => {
      const res =  await request(app).get(`/users/${adminId}`).set('Cookie', `token=${token}`);
      let $ = cheerio.load(res.text);
      csrfToken = $('.downgrade').find('[name=_csrf]').val();
      cookies = res.headers['set-cookie'];
      cookies.push(`token=${token}`);
    };
    ...
    it('should update isAdmin as false when logged in as an admin', async () => {
      await getAdmin();
      const res = await request(app).post('/users/downgrade')
                          .set('Cookie', cookies)
                          .type('form')
                          .send({ id: adminId.toHexString(),
                                  _csrf: csrfToken});
      expect(res.status).toBe(302);
      expect(res.text).toMatch(/dashboard/);
  ...
  describe('GET / new', () => {
    const getNew = async () => {
      return await request(app).get('/users/new').set('Cookie', `token=${token}`);
    };
  ...
  describe('POST /', ()=> {
    const getNewUserForm = async () => {
      ...
      let $ = cheerio.load(res.text);
      csrfToken = $('.userFields').find('[name=_csrf]').val();
      cookies = res.headers['set-cookie'];
      cookies.push(`token=${token}`);
      return res;
    };
  ...
  describe('GET / edit / :id', () => {
    const getEdit = async () => {
      return await request(app).get(`/users/edit/${userId}`).set('Cookie', `token=${token}`);
    };
  ...
  describe('PUT / :id', () => {
    ...
    const getEdit = async () => {
      const res = await request(app).get(`/users/edit/${userId}`).set('Cookie', `token=${token}`);
      let $ = cheerio.load(res.text);
      csrfToken = $('.userFields').find('[name=_csrf]').val();
      cookies = res.headers['set-cookie'];
      cookies.push(`token=${token}`);
