const { makePaidInvoice, makeUnpaidInvoice } = require('../seed/invoice');
...
describe('/reports', () => {
	...
	describe('GET /viewer', () => {
		const sendForm = async () => {
			return await request(app)
				.get(`/reports/viewer?start=${start}&end=${end}`)
				.set('Cookie', `token=${token}`);
		};
		...
		it('should display export link', async () => {
			...
			expect(res.text).toContain(
				`Export received ${moment(start).format('Do MMMM YYYY')} - ${moment(
					end
				).format('Do MMMM YYYY')} to a .csv file`
			);
		});
		...
		it('should display export link', async () => {
			const res = await sendForm();
			expect(res.text).toContain(
				`Export HMRC deductions ${moment(start).format(
					'Do MMMM YYYY'
				)} - ${moment(end).format('Do MMMM YYYY')} to a .csv file`
			);
		});
		...
		it('should redisplay form if invalid parameters sent', async () => {
			const res = await request(app)
				.get(`/reports/viewer?start=tonyHadlee&end=SpandauBallet`)
				.set('Cookie', `token=${token}`);
