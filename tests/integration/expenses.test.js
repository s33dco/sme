...
describe('/expenses', () => {
  ...
  describe('GET /', () => {
    ...
    const exec = async () => {
      return await request(app).get('/expenses').set('Cookie', `token=${token}`);
    };
  ...
  describe('GET / :id', () => {
    ...
    const exec = async () => {
      return await request(app).get(`/expenses/${id}`).set('Cookie', `token=${token}`);
    };
  ...
  describe('POST /', () => {
    ...
    properties = {  date: moment().format('YYYY-MM-DD'),
  ...
  describe('DELETE /', () => {
    ...
    beforeEach( async () => {
      const res = await request(app).get(`/expenses/${id}`).set('Cookie', `token=${token}`);
      let $ = cheerio.load(res.text);
    ...
    const deleteID = async () => {
      return await request(app).delete('/expenses')
          .type('form')
          .set('Cookie', cookies)
          .send(properties);
    };
  ...
  describe('GET / edit / :id', () => {
    const getEdit = async () => {
      return await request(app).get(`/expenses/edit/${id}`).set('Cookie', `token=${token}`);
    };
  ...
  describe('PUT / :id', () => {
    ...
    const getEdit = async () => {
      const res = await request(app).get(`/expenses/edit/${id}`).set('Cookie', `token=${token}`);
      let $ = cheerio.load(res.text);
      csrfToken = $('.expenseFields').find('[name=_csrf]').val();
      cookies = res.headers['set-cookie'];
      cookies.push(`token=${token}`);
      return res;
    };
    ...
    it('updates the record with a valid request', async ()=> {
      ...
      properties = {  date: moment().format('YYYY-MM-DD'),
                      ...
                      amount: '200.00',
                      _csrf : csrfToken}
      const res = await request(app).put(`/expenses/${id}`)
                              .type('form')
                              .set('Cookie', cookies)
                              .send(properties);
      expect(res.status).toBe(302);
