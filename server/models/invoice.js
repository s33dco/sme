	...
	const result = await this.aggregate([
		...
		{ $sort: { 'items.type': -1 } },
		{
			$group: {
				_id: '$items.date',
				item: {
					$push: { type: '$items.type', desc: '$items.desc', fee: '$items.fee' }
				}
			}
		},
		{ $sort: { _id: 1 } }
...
InvoiceSchema.statics.numberOfInvoicesProducedBetween = async function(
	start,
	end
) {
	const result = await this.aggregate([
...
InvoiceSchema.statics.listOfMadeUnpaidInvoicesBetween = async function(
	start,
	end
) {
	return this.aggregate([
