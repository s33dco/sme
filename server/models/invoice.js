...
const itemType = ["Labour", "Materials", "Expense"]
...
InvoiceSchema.statics.itemsByDateAndType = async function(id) {
	const result = await this.aggregate([
		...
		{ $sort: { "items.type": -1 } },
		{
			$group: {
				_id: "$items.date",
				item: { $push: { type: "$items.type", desc: "$items.desc", fee: "$items.fee" } }
			}
		},
		{ $sort: { _id: 1 } }
	])
...
InvoiceSchema.statics.listInvoiceNumbers = async function() {
	const result = await this.aggregate([{ $project: { _id: 1, invNo: 1 } }, { $sort: { invNo: 1 } }])
	return result.map(r => r.invNo)
}

InvoiceSchema.statics.findByInvNo = function(id) {
	return this.aggregate([{ $match: { invNo: id } }, { $project: { _id: 1, invNo: 1 } }])
}
...
InvoiceSchema.statics.numberOfInvoices = async function() {
	const result = await this.aggregate([{ $project: { invNo: 1, _id: 0 } }, { $count: "invoices" }])
