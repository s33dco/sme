...
expenseSchema.statics.listOfficeExpensesBetween = async function (start, end) {
  return await this.aggregate([
    {"$match" : {"date": {"$gte": new Date(start), "$lte": new Date(end)}, "category":"Office, property and equipment"}},
...
expenseSchema.statics.listCarExpensesBetween = async function (start, end) {
  return await this.aggregate([
    {"$match" : {"date": {"$gte": new Date(start), "$lte": new Date(end)}, "category":"Car, van and travel expenses"}},
    {"$project" : { _id:1, date:1, category:1, desc:1, amount:1 }},
    {"$sort": {date:1}}
   ])
};
...
expenseSchema.statics.listLegalExpensesBetween = async function (start, end) {
  return await this.aggregate([
    {"$match" : {"date": {"$gte": new Date(start), "$lte": new Date(end)}, "category":"Legal and financial costs"}},
    {"$project" : { _id:1, date:1, category:1, desc:1, amount:1 }},
    {"$sort": {date:1}}
   ])
};
...
expenseSchema.statics.listMktgExpensesBetween = async function (start, end) {
  return await this.aggregate([
    {"$match" : {"date": {"$gte": new Date(start), "$lte": new Date(end)}, "category":"Mktg, entertainment and subs"}},
    {"$project" : { _id:1, date:1, category:1, desc:1, amount:1 }},
    {"$sort": {date:1}}
   ])
};
...
expenseSchema.statics.listClothingExpensesBetween = async function (start, end) {
  return await this.aggregate([
    {"$match" : {"date": {"$gte": new Date(start), "$lte": new Date(end)}, "category":"Clothing expenses"}},
    {"$project" : { _id:1, date:1, category:1, desc:1, amount:1 }},
    {"$sort": {date:1}}
   ])
};
