...
module.exports = async (req, res, next) => {
	...
	if (!token) {
		...
		throw {
			...
			message:
				"You need to have a valid account to access this area, speak to your administrator if you need these.",
			statusCode: 401
		}
		logger.info(
	...
	try {
		...
		throw {
			...
			message:
				"Maybe you should sign in again and take it from there, your credentials are not valid.",
			statusCode: 401
		}
		logger.info(
			`${e.statusCode} - ${e.tag} - ${e.message} - ${req.originalUrl} - ${req.method} - ${
