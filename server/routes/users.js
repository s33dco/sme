...
router.get('/:id', [auth, admin, validateId], async (req, res) => {
	...
	if (!viewUser) {
		throw {
			...
			message:
				"The user you are looking for cannot be found, maybe it's been deleted, maybe it was never here.",
			statusCode: 404
		};
	}
...
router.delete('/', [auth, admin], async (req, res) => {
	...
	if (req.user._id === req.body.id) {
		throw {
			...
			message:
				'If you want to delete yourself get another admin user to do it for you.',
			statusCode: 400
		};
	}
...
router.get(
	'/edit/:id',
	[auth, admin, validateId, validate.useredit],
	async (req, res) => {
		...
		if (!user) {
			throw {
				...
				message:
					"The user can't be found or edited, maybe you should try again.",
				statusCode: 404
			};
		}
		...
		if (!errors.isEmpty()) {
			...
			if (!updateUser) {
				throw {
					tag: "User can't be found",
					message:
						"The user can't be found to update maybe you should try again.",
					statusCode: 404
				};
			}
			...
			req.flash(
				'success',
				`${updateUser.firstName} ${updateUser.lastName} updated!`
			);
			res.redirect(`/users`);
		}
	}
