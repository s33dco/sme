...
router.post('/', [auth, admin, validate.invoice], async (req, res) => {
	...
	if (!detail) {
		throw {
			...
			message:
				'No information for your invoice can be found, head over to details and add your bank details etc....',
			statusCode: 400
		};
	}
	...
	req.flash(
		'success',
		`Invoice ${invoice.invNo} for ${invoice.client.name} created !`
	);
...
router.get('/:id', [auth, validateId], async (req, res) => {
	...
	if (!invoice) {
		throw {
			tag: 'No longer available.',
			message:
				"The invoice you are looking for cannot be found, maybe it's been deleted, maybe it was never here.",
			statusCode: 404
		};
	}
...
router.post('/email', auth, async (req, res) => {
	if (!ObjectID.isValid(req.body.id)) {
		throw {
			...
			message:
				"The Invoice can't be found to email maybe you should try again.",
			statusCode: 404
		};
	}
	...
	if (!invoice) {
		throw {
			tag: 'No longer available.',
			message:
				'The invoice you want to email cannot be found, maybe it was deleted, maybe it was never here.',
			statusCode: 404
		};
	}
	...
	const total = await Invoice.sumOfInvoice(id);
	const itemsByDateAndType = await Invoice.itemsByDateAndType(id);
	const html4Pdf = await ejs.renderFile('./views/invoicePdf.ejs', {
		total,
		invoice,
		itemsByDateAndType,
		moment: moment
	...
	pdf.create(html4Pdf, optionsPdf).toFile(filePath, function(err, doc) {
		...
		ejs.renderFile(
			'./views/invoiceEmail.ejs',
			{ total, invoice, moment: moment },
			function(error, data) {
				if (error) {
					...
					transporter.sendMail(options, (error, info) => {
						if (error) {
							logger.error(
								`send email error: ${error.message} - ${error.stack}`
							);
							req.flash('alert', `Invoice has not been emailed.`);
							res.redirect('/dashboard');
						} else {
							logger.info(
								`Invoice ${invoice.invNo} mailed to ${invoice.client.email}`
							);
							req.flash(
								'success',
								`Invoice ${invoice.invNo} mailed to ${invoice.client.email}.`
...
router.post('/makepdf', auth, async (req, res) => {
	if (!ObjectID.isValid(req.body.id)) {
		throw {
			tag: "Invoice can't be emailed",
			message:
				"The Invoice can't be found to download maybe you should try again.",
			statusCode: 404
		};
	}
	...
	if (!invoice) {
		throw {
			tag: 'No longer available.',
			message:
				'The invoice you want to downlod cannot be found, maybe it was deleted, maybe it was never here.',
			statusCode: 404
		};
	}
...
router.post('/paid', [auth, admin], async (req, res) => {
	...
	await Invoice.findOneAndUpdate(
		{ _id: req.body.id },
		{ paid: true, $currentDate: { datePaid: true } }
	);

	req.flash(
		'success',
		`Invoice ${invoice.invNo} for ${invoice.client.name} paid!`
	);
	res.redirect('/dashboard');
});
...
router.post('/unpaid', [auth, admin], async (req, res) => {
	...
	req.flash(
		'success',
		`Invoice ${invoice.invNo} for ${invoice.client.name} now unpaid!`
	);
	res.redirect('/dashboard');
});
...
router.get('/edit/:id', [auth, admin, validateId], async (req, res) => {
	...
	if (!invoice) {
		throw {
			...
			message:
				"The invoice can't be found or edited, maybe you should try again.",
			statusCode: 404
		};
	}
	...
	if (invoice.paid) {
		...
		throw {
			tag: "Invoice can't be edited",
			message:
				"The invoice can't be edited once it has been paid, if you need to edit you'll need mark the invoice as unpaid first.",
			statusCode: 400
		};
	}
...
router.put(
	'/:id',
	[auth, admin, validateId, validate.invoice],
	async (req, res) => {
		let errors = validationResult(req);
...
router.delete('/', [auth, admin], async (req, res) => {
	...
	if (invoice.paid) {
		...
		throw {
			tag: "Invoice can't be deleted",
			message:
				"The invoice can't be deleted if it has been paid, if you need to delete you'll need mark the invoice as unpaid first.",
			statusCode: 400
		};
	}
